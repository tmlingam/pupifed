...
class UserController < ApplicationController
    ...
    if request.post?
      ...
      if User.authenticate?(@user.username, params[:user][:old_password])
        if params[:user][:new_password] == params[:user][:confirm_password]
          ...
          #moodle integration
          create_string = 'http://moodle.pupilcentral.in/fedena.php?action=UpdateAccountPassword'
          create_string << "&username=#{@user.username}"
          create_string << "&password=#{params[:user][:new_password]}"
          uri = URI.parse(create_string)
  ...
  def create
    ...
    if request.post?
      ...
      if @user.save
        #moodle integration
        create_string = 'http://moodle.pupilcentral.in/fedena.php?action=CreateAccount'
        create_string << "&username=#{@user.username}"
        create_string << "&password=#{params[:user][:password]}"
        create_string << "&email=#{@user.email}"
        create_string << "&firstname=#{@user.first_name}"
        create_string << "&lastname=#{@user.last_name}"
        uri = URI.parse(create_string)
        Net::HTTP.get_response(uri)
  ...
  def edit
    ...
    #moodle integration
    create_string = 'http://moodle.pupilcentral.in/fedena.php?action=UpdateAccount'
    create_string << "&oldusername=#{@user.username}"
